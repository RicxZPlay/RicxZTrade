...
const BTC_EXTRA_MA_COLORS = ["#60a5fa", "#a78bfa", "#38bdf8"];
...
const DESKTOP_RENDER_INTERVAL_MS = 2000;
const MOBILE_RENDER_INTERVAL_MS = 2000;
...
const BTC_ONE_MINUTE_CHART_ID = "candles-1m";
const BTC_MAIN_CHART_IDS = [BTC_ONE_MINUTE_CHART_ID, BTC_CANDLE_CHART_ID];
const TOOLS = {
  cursor: "cursor",
  trend: "trend",
  ruler: "ruler",
...
export default function BtcQuadView({ embedded = false, onClose, onFullscreen, theme }) {
  ...
  const visibleCharts = useMemo(
    () => BTC_MAIN_CHART_IDS
      .map((id) => BTC_QUAD_CHARTS.find((config) => config.id === id))
      .filter(Boolean),
    []
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      secondaryBandData.forEach((bandData, index) => {
        ...
        syncSeriesData(
          extraBandLineRefs.current[index * 2 + 1],
          bandData?.lower || [],
          seriesSyncRef.current,
          `extraBbLower-${index}`,
          incrementalSync
    ...
    <article className={getChartCardClassName(config)}>
      ...
      <div className="btc-quad-chart-area">
        ...
        <div className="btc-price-scale-hint" aria-hidden="true">
          <ArrowUp className="btc-scale-arrow up" size={22} strokeWidth={2.4} />
          <ArrowDown className="btc-scale-arrow down" size={22} strokeWidth={2.4} />
        </div>
        <svg
          ref={overlayRef}
...
function getChartCardClassName(config) {
  ...
  if (config?.id === "candles-1m") classes.push("btc-quad-card-normal-1m");
...
function getChartExtraBollingerBands(config) {
