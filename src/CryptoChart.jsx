...
export default function CryptoChart({ symbol, candles, monthlyCandles = [], liveStatus, error, theme, mode = CHART_MODES.btc, timeframe = DEFAULT_BTC_RENKO_TIMEFRAME }) {
  ...
  useEffect(() => {
    ...
    const bands = isAltChart ? { upper: [], middle: [], lower: [] } : toChartBollingerBands(candles, btcBoxSize);
  ...
  }, [altFastLsma, altMa, btcBoxSize, candles, chartData, isAltChart, symbol]);
...
function clipLineToChartData(line, chartData) {
  if (!Array.isArray(line) || chartData.length === 0) return [];
